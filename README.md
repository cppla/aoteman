...
| 阶段 | 规则 |
| --- | --- |
| 濒危求援 | 生命 ≤30%，至少 10 光能时，点「召唤伙伴」或按 **6**；每场一次 |
| 双伙伴登场 | 耗尽现有光能，打断怪兽蓄力，恢复最大生命的 25%（向上取整），获得 3 秒免伤护盾 |
| 迪迦援护 | 登场 0.8 秒后出击，每 2.4 秒交替拳击（12 伤害）与光线（18 伤害） |
| 赛罗援护 | 登场 1.1 秒后出击，每 2.8 秒交替拳击（14 伤害）与光线（20 伤害） |
| 三重必杀 | 援护按钮变成「三重必杀」，消耗 30 光能，共造成 84 伤害，冷却 8 秒；三人同步光线姿势持续 1.1 秒，普通攻击共享 1.2 秒出招冷却 |

合击后，两位伙伴的自动攻击分别从 2.4 / 2.8 秒重新计时。护盾结束后仍需防御；生命归零后不能召唤。伤害不会超过怪兽剩余生命。胜负确定时立即保存奖励，致胜必杀会完整播放，再展示结果。

**成长会回应**：训练、每日礼物、成长航线和战斗获得经验时，银河会敬礼、举臂，显示经验勋章；跨级时展开柔和光环与星光，告诉你新的等级和成长称号。战斗成长回应等返回基地后播放，刷新不重复播放，也不重复发奖励。摸摸会得到轮换的亲昵回应。

**iPad 触控适配**：战斗布局随 Safari 可用高度变化；iPad 横竖屏、浏览器工具栏缩短视口和分屏时，六个技能与暂停/退出都留在屏内。触控按钮至少 44px，保留双指缩放；旋转不重开战斗，离开页面自动暂停。动作增加拳风、命中星芒、三色脚下光环和必杀速度线，并尊重系统的减少动态效果设置。

存档仍使用兼容的 v2 格式和 SQLite 结构版本 2。服务器数据库保存在 Docker 的 `aoteman_data` 卷；版本更新先备份，再更换容器，不导入本地旧进度。

![三位奥特曼同时释放必杀](docs/screenshots/trio-ipad.png)

![升级时银河举臂回应](docs/screenshots/growth-response.png)
...
- **濒危援护**：用最后的光能召唤迪迦和赛罗，独立援护与三重必杀让三人共同击退怪兽。
- **技能快捷键**：数字 **1–5** 对应原有战斗技能，**6** 召唤 / 三重必杀，**P** 暂停，也支持点击和触摸。
...
v3 存档验证记录见 [validation-v3.md](docs/validation-v3.md)；成长与训练见 [validation-v4.md](docs/validation-v4.md)；v5 双人援护见 [validation-v5.md](docs/validation-v5.md)；本轮三人战斗与 iPad 适配见 [validation-v6.md](docs/validation-v6.md)。
